...
Now that your agent is refined and tested, let's publish it as an Agent Application. Publishing creates a stable, managed endpoint that external applications can call — separate from your development project.

1. In the Agent Builder in VS Code (where you were just testing), select **Publish Agent**.

1. In the publish dialog, keep the default authentication type of **RBAC** (Role-Based Access Control). This means callers must have the **Azure AI User** role on the Agent Application resource.

1. Wait for publishing to complete. Your agent version will show a published status.

1. After publishing, note the values you'll need to construct the endpoint URL:

    - **Foundry resource name**: The name of your Azure AI Foundry resource (e.g., `my-foundry-resource`)
    - **Project name**: Your Foundry project name (e.g., `it-support-agent-project`)
    - **Application name**: The name assigned to the Agent Application when published

    The published endpoint follows this format:

    ```
    https://<foundry-resource-name>.services.ai.azure.com/api/projects/<project-name>/applications/<app-name>/protocols/openai
    ```

1. Ensure your Azure account has the **Azure AI User** role on the Agent Application resource. This is required to invoke the published endpoint. You can verify this in the Azure portal under the Agent Application resource's **Access control (IAM)** settings.

    > **Note**: Published agents use a stateless API. The web application manages conversation history locally, sending the full context with each request.

## Build a web chat application

Now that your agent is published with a stable endpoint, let's connect it to a real web application. You'll run a Flask-based chat app that provides a browser UI for interacting with your published agent.
...
1. In the Explorer pane, you'll see the following project structure:
    ...
    - `app.py` — The Flask backend that connects to your published agent
...
1. Open `app.py` and review the key components:
    ...
    - **`get_openai_client()`** — Lazily initializes the OpenAI client using your published agent endpoint and Azure credentials. The app only connects to Azure when the first request comes in, making startup errors easier to diagnose.
    - **`/api/conversation`** (POST) — Creates a new local session to track conversation history. Published agents use a stateless API, so the app manages conversation history server-side.
    - **`/api/chat`** (POST) — Appends the user message to the session history, sends the full conversation to the published agent, and returns the response.
    - **`extract_response()`** — Parses the agent's response to extract text and source citations from file search.

1. Open `static/js/chat.js` and notice how the frontend:

    - Creates a session when the page loads
...
The app has two `TODO` sections that you need to complete before it can connect to your published agent.

1. In `app.py`, find the `get_openai_client()` function. Replace the `# TODO` comment with the following code to create the authentication token provider and initialize the OpenAI client:
    ...
    This code uses `DefaultAzureCredential` to authenticate with Azure and creates an OpenAI client that points to your published agent's endpoint. The `api_key` parameter accepts the token provider, which automatically refreshes tokens as needed.

1. Next, find the `chat()` function. Replace the `# TODO` comment and the placeholder `return` statement with the following code:

    ```python
        # Send full conversation history (stateless Responses API)
    ...
    Because published agents use a stateless API, this code sends the *entire* conversation history as the `input` parameter each time. It then stores the assistant's reply so that future turns include prior context — enabling multi-turn conversations.
...
1. In the `.env` file, replace `your_agent_app_endpoint_here` with the published endpoint URL you constructed earlier:

    ```
    AGENT_APP_ENDPOINT=https://<foundry-resource-name>.services.ai.azure.com/api/projects/<project-name>/applications/<app-name>/protocols/openai
    ```

    Replace `<foundry-resource-name>`, `<project-name>`, and `<app-name>` with your actual values.
...
With the chat app running in your browser, test the agent capabilities through the published endpoint.
...
2. Test **code interpreter** with a data analysis request:
    ...
    The agent will use code interpreter to analyze the CSV data and provide text-based insights and statistics.

3. Try a **combined query** that exercises both tools:
...
4. Test **multi-turn conversation** by asking a follow-up question:

    ```
    Can you summarize the key findings from your analysis?
    ```

    The agent remembers the conversation context because the app sends the full history with each request.
...
Observe how the web application provides an interactive experience — rendering markdown-formatted responses and displaying source citations from grounding data. This is the same agent you built and tested in the portal and VS Code, now published as a managed endpoint and accessible through a real application.

Press **Ctrl+C** in the terminal to stop the web server when you're done testing.

## Summary

In this exercise, you built an AI agent in the Microsoft Foundry portal with file search and code interpreter tools, refined it using the AI Toolkit extension in VS Code, published it as an Agent Application with a stable endpoint, and connected it to a Flask-based web chat application.
